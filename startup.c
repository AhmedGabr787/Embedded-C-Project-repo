...
void Reset_Handler(void)
{
	...
	uint32_t data_size = (uint32_t)&_edata_ - (uint32_t)&_sdata_;
	uint8_t* SRC = (uint8_t*)&_etext_;
	uint8_t* DEST = (uint8_t*)&_sdata_;
	for(uint32_t i=0;i<data_size;i++){
		*DEST++ = *SRC++;
}
	//reserve .bss section and initialise it with zero
	data_size = (uint32_t)&_ebss_ - (uint32_t)&_sbss_;
	DEST = (uint8_t*)&_sbss_;
	for(uint32_t i=0;i<data_size;i++){
	*DEST++ = 0U;
}
